...
class Notice{
    ...
    public function  generate_html() {
        ...
        <div
            ...
            class="notice wpmet-notice notice-<?php echo esc_attr($this->notice_id . ' ' .$this->class); ?> <?php echo esc_attr(false === $this->dismissible ? '' : 'is-dismissible') ;?>"

            expired_time="<?php echo esc_attr($this->expired_time); ?>"
            dismissible="<?php echo esc_attr($this->dismissible); ?>"
        >
            <?php if(!empty($this->logo)):?>
                <img class="notice-logo" style="<?php echo esc_attr($this->logo_style); ?>" src="<?php echo esc_url($this->logo);?>" alt="<?php esc_attr_e('Notice Logo','shopengine-gutenberg-addon'); ?>"/>
            <?php endif; ?>

            <div class="notice-right-container <?php echo (empty($this->logo) ? esc_attr('notice-container-full-width') : esc_attr('')); ?>">
                    ...
                    <?php if(!empty($this->buttons)): ?>
                        <div class="button-container notice-vert-space">
                            <?php foreach($this->buttons as $button): ?>
                                <a id="<?php echo (!isset($button['id']) ? esc_attr('') : esc_attr($button['id'])); ?>" href="<?php echo esc_url($button['url']); ?>" class="wpmet-notice-button <?php echo esc_attr($button['class']); ?>">
    ...
    public static function dismiss_ajax_call() {
        ...
        if( ! isset( $_POST['wp_nonce'] ) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['wp_nonce'])), 'shopengine_notice_nonce') ){
                    ...
                    $( '.wpmet-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                wp_nonce        : '". esc_html(wp_create_nonce('shopengine_notice_nonce')) ."'
                            },
                        });
                    });
                });
