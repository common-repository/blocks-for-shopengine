...
$vacation_days = $module_list->get_list()['vacation']['settings']['vacation_days']['value']; 

$today_date = date('Y-m-d'); 
// Check if today is within any vacation days range
$in_vacation_period = false;
foreach ($vacation_days as $vacation_period) {
    $start_date = $vacation_period['start_and_end_date'][0];
    $end_date = $vacation_period['start_and_end_date'][1];

    // Check if today falls between the start and end dates
    if ($today_date >= $start_date && $today_date <= $end_date) {
        $in_vacation_period = true;
        break;
    }
}
